...
| Skill | What it does |
|---|---|
| `/cpp-guidelines` | The C++23 standards and load-bearing design principles used across these projects — `std::expected` error handling, dependency injection, configuration at construction time, `enum class` over `bool` in API surface, binary literals for bit patterns, data-driven design, testability, zero-warning policy. Defers to each project's `.clang-tidy` and `AGENT.md`. |
