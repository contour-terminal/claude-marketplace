...
true and every lane holding exactly one ticket.
...
- `${CLAUDE_PLUGIN_ROOT}/lib/team-protocol.md` — roles, lanes, the claim, the brief, review gates,
  merging, intake. This is the one you will use most.
...
Have the developer invoke `/sprint-dev <issue> <lane>`. That is the point of it existing: the
...
Set `Status` to `In Progress`. **Do not** record which session took it anywhere on the item; the
branch is the claim. See `lib/team-protocol.md` §*The branch is the claim*.
...
Then set the item `Done`, and check whether anything `Blocked` on it can move to `Todo`.
